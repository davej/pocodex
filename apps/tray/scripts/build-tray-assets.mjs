...
if (existsSync(sipsPath)) {
  ...
  ensureGeneratedAssetsExist([
    ...trayIconSizes.map((size) => join(assetsDirectory, `tray-template-${size}.png`)),
    ...appIconSizes.map((size) => join(assetsDirectory, `app-icon-${size}.png`)),
  ]);
}
...
function ensureGeneratedAssetsExist(paths) {
  ...
  throw new Error(
    `Missing generated tray assets and ${sipsPath} is unavailable: ${missingPaths.join(", ")}`,
  );
}
